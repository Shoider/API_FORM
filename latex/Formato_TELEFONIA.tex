%\textbf{\emph{ejemplo}}
\input{Datos.txt}
\documentclass[letterpaper,11pt]{article}
\usepackage{ifthen}

% *** Idioma ***
\usepackage[spanish,es-nodecimaldot]{babel}
\usepackage[utf8]{inputenc}

% *** Fuente ***
%\usepackage{lmodern}
%\usepackage[T1]{fontenc}
%%%FUENTE NOTO SANS
\usepackage{fontspec}
\setmainfont{Noto Sans}


% *** Colores ***
\usepackage{xcolor}
\definecolor{dorado}{RGB}{190,150,91} 
\definecolor{gris}{rgb}{0.752,0.752,0.752}

% *** Margenes y Tamaño de Hoja
\usepackage{geometry}
\geometry{
left = 15mm,
right = 15mm,
top = 45mm,
bottom = 20mm,
headheight = 30mm
}

% *** Imagenes ***
\usepackage{graphicx}
\graphicspath{./imagenes}
\usepackage{float}
\usepackage{subcaption}
\usepackage{wrapfig}
\usepackage{tabularray}

% *** Fecha ***
%\usepackage[useregional]{datetime2}
\usepackage{datetime2}
\DTMsetdatestyle{ddmmyyyy}

%No cortar palabras
\usepackage[none]{hyphenat}


% *** Encabezado ***
\usepackage{lastpage} 
\usepackage{fancyhdr}
\pagestyle{fancy}
\renewcommand{\headrulewidth}{0pt}

\lhead{
		\begin{tblr}{
		width = \linewidth,
		colspec = {Q[120]Q[507]Q[70]Q[70]},
		row{2} = {c},
		row{3} = {c},
		column{2} = {c},
		cell{1}{1} = {r=4}{c},
		cell{1}{2} = {r=3}{},
                     cell{4}{2} = {h},
		cell{1}{3} = {c},
		cell{1}{4} = {c},
		cell{4}{3} = {c=2}{c},
		vlines,
                      hlines,
	}
	\includegraphics[width=0.15\textwidth]{imagenes/Logo_de_la_Conagua} & {\textbf{COMISIÓN NACIONAL DEL AGUA}\\Gerencia de Tecnologías de Información y Comunicaciones.\\ Subgerencia de Soporte Técnico, Telecomunicaciones y Seguridad.} & \footnotesize Página    & {\footnotesize \thepage\ de \pageref{LastPage}} \\
	&                                                                                       & \footnotesize VERSIÓN   & \footnotesize 5.1 \\
	&                                                                                       & \footnotesize FECHA & {\footnotesize  Mayo-2025 }    \\
	& {\textbf{ Solicitud de servicios de telefonía.}\\\textbf{(Alta, Baja, Cambio)}}                                       &  \includegraphics[width=0.05\textwidth]{imagenes/Logo_Octubre_2024}       &           
\end{tblr}
}
\chead{}
\rhead{}

% *** Pie de Pagina
%\renewcommand{\footrulewidth}{0.5pt}
\lfoot{\tiny Notas: a) La información de este documento implica conocer y acepatr la normatividad vigente, así como el compromiso de uso ético y racional de los servicios asignados. b)En caso de impresiones en hojas separadas, todas deben venir rubrica. }
\cfoot{}
%\rfoot{}

\begin{document}
\sloppy

\begin{longtblr}[
	label = none,
	entry = none,          
	]{
		width = \linewidth,
		colspec = {Q[203]Q[200]Q[200]Q[200]},
		column{2} = {c},
                     column{4} = {c},
                     cell{3}{2}={c},
                     cell{3}{4}={c},
                     cell{4}{4}={c},
                     cell{2}{3} = {c},
		cell{2}{4} = {c},
	}
	Número de formato:  & \NOFORMATO          & Fecha de la solicitud:   & \today       \\
	Tipo de Movimiento: & Alta (\ALTA ) & Baja (\BAJA ) & Cambio (\CAMBIO ) \\
           Tipo de usuario: & \TIPOUSUARIO \\
           Fecha de activación: & \ACTIVACION &Fecha de expiración: & \EXPIRACION
\end{longtblr}
\vspace{-30pt}
\begin{longtblr}[
	label = none,
	entry = none,
	]{
		width = \linewidth,
		colspec = {Q[5]Q[5]},
                     %row{1}={dorado},   
		column{1}={r},
		column{2}={l},    
		row{1} = {c},             
		cell{1}{1} = {c=2}{0.874\linewidth},
		hlines,
		vlines,
	}
\textbf{Datos de Usuario (a) que utilizará el servicio (CONAGUA o Externo)} &   \\
Nombre:                     &  \NOMBREUSUARIO \\
Correo: &   \CORREOUSUARIO  \\
Dirección:   &  \textbf  \DIRECCION  \\
Puesto: & \PUESTOUSUARIO\\
Unidad Aministrativa:           &\UAUSUARIO
\end{longtblr}

\newboolean{externo}
\setboolean{externo}{\EXTERNO}
\ifthenelse{\boolean{externo}}
{
\vspace{-25pt}
\begin{longtblr}[
	label = none,
	entry = none,
	]{
		width = \linewidth,
		colspec = {Q[5]Q[5]},
		column{1}={r},
		column{2}={l},
		row{1} = {c},
                     %row{1}={dorado},                    
		cell{1}{1} = {c=2}{0.874\linewidth},
		%cell{4}{2} = {c=3}{0.674\linewidth},
		%cell{5}{2} = {c=3}{0.674\linewidth},        
		hlines,
		vlines,
	}
\textbf{Datos del Empleado(a) de CONAGUA (Responsable, si el usuario es externo)} &   \\
Nombre:                     &  \NOMBREEMPLEADO \\
Número de empleado:&\IDEMPLEADO \\
Correo: & \CORREOEMPLEADO \\
Telefóno/Ext: & \EXTEMPLEADO  \\
Puesto:           &\PUESTOEMPLEADO
\end{longtblr}
}

\vspace{-25pt}
\begin{longtblr}[
	label = none,
	entry = none,
	]{
		width = \linewidth,
		colspec = {Q[3]Q[2]Q[3]Q[2]},		
                     %row{1}={dorado},
		column{1}={r},
		column{2}={c},
		column{3}={r},
		column{4}={c},
		row{1} = {c},
                    cell{1}{1} = {c=4}{0.874\linewidth},		
                    cell{5}{1}={c=3}{0.674\linewidth},
                     %cell{5}{3}={c=2}{0.374\linewidth},
		hlines,
		vlines,
	}
\textbf{Caracteristicas del Servicio Solicitado}         \\
\textbf{Servicio Interno:}       &  Si (X)~ No(~ )  & \textbf{\textbf{Serivicio Celular Local y Foráneo:}}                           & Si (\SICELULAR )~ No(\NOCELULAR ) \\
	\textbf{Servcio Larga Distancia Nacional:}      &  Si (\SINACIONAL )~ No(\NONACIONAL)  &\textbf {Servicio Larga Dsitancia Internacional:}    & Si (\SIMUNDO )~ No(\NOMUNDO ) 
	
\end{longtblr}
\vspace{-25pt}
\begin{longtblr}[
	label = none,
	entry = none,
	]{
		width = \linewidth,
		colspec = {Q[50]Q[50]Q[50]Q[50]Q[50]},
		row{1} = {c},
                     row{2}={c},
                     row{3}={c},
                     %row{1}={dorado},
                     cell{1}{1} = {c=5}{},		
		hlines,
		vlines,
	}
\textbf{Caracteristicas del Equipo donde se configura la cuenta de Red}         \\
 \textbf {Tipo Equipo}&\textbf {Marca}&\textbf {Modelo}&\textbf {Serie}&\textbf {Versión de sistema operativo}\\
TELÉFONO&\MARCA&\MODELO&\SERIE&\VERSION
\end{longtblr}

\vspace{-30pt}
\begin{longtblr}[
	label = none,
	entry = none,
	]{
		width = \linewidth,
		colspec = {Q[935]},
                    % row{1}={dorado},
		cells = {c},
		row{2} = {},
		hlines,
		vlines,
	}
	\textbf{Justificación de los Servicios Solicitados} \\\JUSTIFICACION
\end{longtblr}
\clearpage
%\vspace{-20pt}
\textbf{Términos y Condiciones del servicio:}
PENDIENTES
\begin{longtblr}[
	label = none,
	entry = none,
	]{
			width = \linewidth,
			colspec = {Q[400]Q[400]},
			%row{1} = {gris},
			row{1} = {c},
			%cell{1}{1} = {c=2}{0.874\linewidth},
			%hlines,
			%vlines,
		}
	%	Nombres Cargos y Firmas                      &                                                                                                       \\
		{Usuario(a) del servicio\\{ \tiny He leído y acepto la normatividad vigente}
                      \\[0.5cm] \rule{6cm}{0.5mm}\\\NOMBREUSUARIO\\\PUESTOUSUARIO} & 
                      {Autoriza \\{ \tiny  \PUESTOJEFE}
                      \\[0.5cm] \rule{6cm}{0.5mm}\\\NOMBREJEFE\\\PUESTOJEFE} 
\end{longtblr}

\end{document}