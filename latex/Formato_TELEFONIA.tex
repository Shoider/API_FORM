%\textbf{\emph{ejemplo}}
\input{Datos.txt}
\documentclass[letterpaper,9pt]{article}
\usepackage{ifthen}

% *** Idioma ***
\usepackage[spanish,es-nodecimaldot]{babel}
\usepackage[utf8]{inputenc}

% *** Fuente ***
\usepackage{lmodern}
\usepackage[T1]{fontenc}


% *** Colores ***
\usepackage{xcolor}
\definecolor{dorado}{RGB}{190,150,91} 
\definecolor{gris}{rgb}{0.752,0.752,0.752}

% *** Margenes y Tamaño de Hoja
\usepackage{geometry}
\geometry{
left = 25mm,
right = 25mm,
top = 60mm,
bottom = 20mm,
headheight = 28mm
}

% *** Imagenes ***
\usepackage{graphicx}
\graphicspath{./imagenes}
\usepackage{float}
\usepackage{subcaption}
\usepackage{wrapfig}
\usepackage{tabularray}

% *** Fecha ***
%\usepackage[useregional]{datetime2}
\usepackage{datetime2}
\DTMsetdatestyle{ddmmyyyy}

%No cortar palabras
\usepackage[none]{hyphenat}


% *** Encabezado ***
\usepackage{lastpage} 
\usepackage{fancyhdr}
\pagestyle{fancy}
\renewcommand{\headrulewidth}{0pt}

\lhead{
		\begin{tblr}{
		width = \linewidth,
		colspec = {Q[230]Q[507]Q[100]Q[100]},
		row{2} = {c},
		row{3} = {c},
		column{2} = {c},
		cell{1}{1} = {r=4}{c},
		cell{1}{2} = {r=3}{},
                     cell{4}{2} = {h},
		cell{1}{3} = {c},
		cell{1}{4} = {c},
		cell{4}{3} = {c=2}{c},
		vlines,
                      hlines,
	}
	\includegraphics[width=0.2\textwidth]{imagenes/Logo_de_la_Conagua} & {Comisión Nacional del Agua.\\Gerencia de Tecnologías de Información \\y Comunicaciones.\\ Subgerencia de Soporte Técnico, \\ Telecomunicaciones y Seguridad.} & \tiny Hoja    & {\tiny \thepage\ de \pageref{LastPage}} \\
	&                                                                                       & \tiny VERSIÓN   & \tiny 5.1 \\
	&                                                                                       & \tiny FECHA & {\tiny FECHA DE PUBLICACIÓN PENDIENTE }    \\
	& { Solicitud de servicios de telefonía.\\(Alta, Baja, Cambio)}                                       &  \includegraphics[width=0.05\textwidth]{imagenes/Logo_Octubre_2024}       &           
\end{tblr}
}
\chead{}
\rhead{}

% *** Pie de Pagina
%\renewcommand{\footrulewidth}{0.5pt}
\lfoot{\tiny Notas: a) La información de este documento implica conocer y aceptar la normatividad vigente, así como el compromiso de uso ético y racional de los servicios asignados. b)En caso de impresiones en hojas separadas, todas deben venir rubrica. }
\cfoot{}
%\rfoot{}

\begin{document}
\sloppy

\begin{longtblr}[
	label = none,
	entry = none,          
	]{
		width = \linewidth,
		colspec = {Q[203]Q[200]Q[200]Q[200]},
		column{2} = {c},
                     column{4} = {c},
                     cell{3}{2}={c},
                     cell{3}{4}={c},
                     cell{4}{4}={c},
                     cell{2}{3} = {c},
		cell{2}{4} = {c},
	}
	Número de formato:  & \NOFORMATO          & Fecha de la solicitud:   & \today       \\
	Tipo de movimiento: & Alta (\ALTA ) & Baja (\BAJA ) & Cambio (\CAMBIO ) \\
           Tipo de usuario: & \TIPOUSUARIO \\
           Fecha de activación: & \ACTIVACION &Fecha de expiración: & \EXPIRACION
\end{longtblr}
\vspace{-30pt}
\begin{longtblr}[
	label = none,
	entry = none,
	]{
		width = \linewidth,
		colspec = {Q[90]Q[300]},
		row{1} = {c},
                     row{1}={dorado},                    
		cell{1}{1} = {c=2}{0.874\linewidth},
		hlines,
		vlines,
	}
\textbf{Datos de Usuario(a) que utilizará el servicio (CONAGUA o Externo)} &   \\
Nombre:                     &  \NOMBREUSUARIO \\
Correo: &   \CORREOUSUARIO  \\
Dirección:   & \textbf  \DIRECCION  \\
Puesto: & \PUESTOUSUARIO\\
Unidad Administrativa:           &\UAUSUARIO
\end{longtblr}

\newboolean{externo}
\setboolean{externo}{\EXTERNO}
\ifthenelse{\boolean{externo}}
{
\vspace{-30pt}
\begin{longtblr}[
	label = none,
	entry = none,
	]{
		width = \linewidth,
		colspec = {Q[100]Q[300]Q[200]Q[300]},
		row{1} = {c},
                     row{1}={dorado},         
                     cell{3}{3}={j},           
		cell{1}{1} = {c=4}{0.874\linewidth},
		cell{4}{2} = {c=3}{0.674\linewidth},      
		hlines,
		vlines,
	}
\textbf{Datos del Empleado(a) de CONAGUA (Responsable, si el usuario es externo)} &  &      &  \\
Nombre:                     &  \NOMBREEMPLEADO &Número de empleado:&\IDEMPLEADO \\
Correo: & \CORREOEMPLEADO &Teléfono/Ext: & \EXTEMPLEADO  \\
Puesto:           &\PUESTOEMPLEADO
\end{longtblr}
}

\vspace{-30pt}
\begin{longtblr}[
	label = none,
	entry = none,
	]{
		width = \linewidth,
		colspec = {Q[220]Q[160]Q[220]Q[160]},
		row{1} = {c},
                     row{1}={dorado},
                    cell{1}{1} = {c=4}{0.874\linewidth},		
                    cell{5}{1}={c=3}{0.674\linewidth},
                     %cell{5}{3}={c=2}{0.374\linewidth},
		hlines,
		vlines,
	}
\textbf{Características del Servicio Solicitado}         \\
\textbf{Servicio Interno:}       &  Si (X)~ No(~ )  & \textbf{\textbf{Servicio de Larga \\Distancia\\Resto del Mundo:}}                           & Si (\SIMUNDO )~ No(\NOMUNDO ) \\
	\textbf{Servicio Local:}      &  Si (\SILOCAL )~ No(\NOLOCAL)  &\textbf {Servicio Celular Local:}    & Si (\SICLOCAL )~ No(\NOCLOCAL ) \\
	\textbf{Servicio Larga Distancia Nacional:}       &Si (\SINACIONAL )~ No(\NONACIONAL ) & \textbf{Servicio Celular Larga Distancia:}                                  & Si (\SICNACIONAL )~ No(\NOCNACIONAL ) \\
	\textbf{Servicio Larga Distancia Estados Unidos y Canadá:} &&& Si (\SIEUA )~ No(\NOEUA)
\end{longtblr}
\vspace{-30pt}
\begin{longtblr}[
	label = none,
	entry = none,
	]{
		width = \linewidth,
		colspec = {Q[50]Q[50]Q[50]Q[50]Q[50]},
		row{1} = {c},
                     row{2}={c},
                     row{3}={c},
                     row{1}={dorado},
                     cell{1}{1} = {c=5}{},		
		hlines,
		vlines,
	}
\textbf{Características del Equipo donde se configura la cuenta de Red}         \\
 \textbf {Tipo Equipo}&\textbf {Marca}&\textbf {Modelo}&\textbf {Serie}&\textbf {Versión de sistema operativo}\\
Teléfono&\MARCA&\MODELO&\SERIE&\VERSION
\end{longtblr}

\vspace{-30pt}
\begin{longtblr}[
	label = none,
	entry = none,
	]{
		width = \linewidth,
		colspec = {Q[935]},
                     row{1}={dorado},
		cells = {c},
		row{2} = {},
		hlines,
		vlines,
	}
	\textbf{Justificación de los Servicios Solicitados} \\\JUSTIFICACION
\end{longtblr}
\clearpage
%\vspace{-20pt}
\begin{longtblr}[
	label = none,
	entry = none,
	]{
			width = \linewidth,
			colspec = {Q[400]Q[400]},
			%row{1} = {gris},
			row{1} = {c},
			%cell{1}{1} = {c=2}{0.874\linewidth},
			%hlines,
			%vlines,
		}
	%	Nombres Cargos y Firmas                      &                                                                                                       \\
		{Usuario(a) del servicio\\{ \tiny He leído y acepto la normatividad vigente}
                      \\[0.5cm] \rule{6cm}{0.5mm}\\\NOMBREUSUARIO\\\PUESTOUSUARIO} & 
                      {Autoriza \\{ \tiny  \PUESTOJEFE}
                      \\[0.5cm] \rule{6cm}{0.5mm}\\\NOMBREJEFE\\\PUESTOJEFE} 
\end{longtblr}

\end{document}